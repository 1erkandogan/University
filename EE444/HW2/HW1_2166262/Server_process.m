...
function reply(srv, packet)
    ...
    elseif op == "DEL"
        table(:, indices) = 0;
        response = "OP=DEL;SUCCESS;";
    elseif op == "SRH"
        temp = split(fields(2),'=');
        temp = temp(2);
        search_data = str2double(temp);
        found_indices = find(table(2, :) == search_data) - 1; % Subtract 1 for 0-based indexing
    
        if ~isempty(found_indices)
            response = strcat("OP=SRH;IND=", join(string(found_indices), ','), ";");
        else
            response = "OP=SRH;IND=;";
        end
